...
trait Buildingblock {
  ...
  def _parameterGradient(parameters: Array[Double], inputs: Array[Double], calculatedOutputs: Array[Double],
    outputDerivative: Array[Double]): Array[Double]
  ...
  def _inputGradient(parameters: Array[Double], inputs: Array[Double], calculatedOutputs: Array[Double],
    outputDerivative: Array[Double]): Array[Double]
  ...
  /** For efficient calculation of gradients for evaluation of a chain of units.
    ...
    */
  def _gradientChain(parameters: Array[Double],
    chain: Array[Double] => (Double, Array[Double], Array[Double])): Array[Double] => (Double, Array[Double], Array[Double]) = {
    ...
    (inputs: Array[Double]) => {
      ...
      (evaluation, _inputGradient(parameters, inputs, outputs, evalInputGradient),
        _parameterGradient(parameters, inputs, outputs, evalInputGradient) ++ evalParameterGradient)
    }
  }
  ...
  /** Calculates an evaluation and its gradients.
    ...
    */
  def gradient(parameters: Array[Double], inputs: Array[Double],
    evalWithGradient: Array[Double] => (Double, Array[Double])): (Double, Array[Double]) = {
    require(inputs.length == numInputs, "inputs " + inputs.length + " but expected " + numInputs)
    require(parameters.length == numParameters, "parameters " + parameters.length + " but expected " + numParameters)
    val chain = evalWithGradient.andThen(eg => (eg._1, eg._2, Array[Double]()))
    _gradientChain(parameters, chain).andThen(g => (g._1, g._3))(inputs)
  }

  def evalWithGradientAndDirectionalDerivations(parameters: Array[Double], inputs: Array[Double],
    outputGradDD: Array[Double] => (Double, Array[Double], (Array[Double] => (Double, Double))))
  : (Double, Array[Double], Double, Double) = {
    val outputs: Array[Double] = apply(parameters)(inputs)
    val (outevl, outgrad, outDD) = outputGradDD(outputs)
    val (evl, grad, dd) = _evalGradDdChain(parameters, inputs)((outevl, outgrad, outDD))
    val ddevl = dd(grad)
    (evl, grad, ddevl._1, ddevl._2)
  }

  def _evalGradDdChain(parameters: Array[Double], inputs: Array[Double])
    (rest: (Double, Array[Double], (Array[Double] => (Double, Double)))):
  (Double, Array[Double], (Array[Double] => (Double, Double))) = ???
}
...
case class Combined(first: Buildingblock, second: Buildingblock) extends Buildingblock {
  ...
  override def _parameterGradient(parameters: Array[Double], inputs: Array[Double], calculatedOutputs: Array[Double],
    outputDerivative: Array[Double]): Array[Double] = ???

  /** not implemented since it would be horribly inefficient and gradientChain works otherwise. */
  override def _inputGradient(parameters: Array[Double], inputs: Array[Double], calculatedOutputs: Array[Double],
    outputDerivative: Array[Double]): Array[Double] = ???
  ...
  def _gradientChain(parameters: Array[Double],
    ...
    require(parameters.length == numParameters, "parameters " + parameters.length + " but expected " + numParameters)
    val parameterSplitted = parameters.splitAt(first.numParameters)
    first._gradientChain(parameterSplitted._1, second._gradientChain(parameterSplitted._2, chain))
  }
